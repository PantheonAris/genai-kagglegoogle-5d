# Multi-Agent Financial Advisory System: Requirements (v2)
...
- **FR-FA-2:** Must include a **Stock Data and Analysis Tool**:
    - Fetches real-time and historical stock data for specified tickers (e.g., AAPL, TSLA) using multiple data sources, including Alpha Vantage and the Yahoo Finance API, for redundancy and coverage.
...
- **NFR-C-4:** The system should intelligently select the most cost-effective data provider (Alpha Vantage vs. Yahoo Finance) for a given request, where possible.
...
- **NFR-R-4:** The Financial Analysis Agent should implement a fallback mechanism, automatically switching from the primary (e.g., Alpha Vantage) to the secondary (e.g., Yahoo Finance) data source if an API fails.
